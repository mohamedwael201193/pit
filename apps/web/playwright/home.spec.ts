...
test("home is intelligence, not a wallet-first landing", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { name: /Your trading agent doesn't sleep/ })).toBeVisible();
  await expect(page.getByText("Your keys don't leave your machine.")).toBeVisible();
  ...
  await expect(page.getByRole("link", { name: "Verify a mission" }).first()).toBeVisible();
