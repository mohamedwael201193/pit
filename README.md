...
PIT is a **multi-user trading desk** for 0G + Hyperliquid. It seals **your** private book into **0G Direct TeeML**, runs researcher / challenger / risk over that book, and sizes the order on the **host** (the model cannot set size). Manual mode waits for **you** to authorize an exact preview. A Sleep Mission is optional bounded host execution and only arms from this computer — chat, the website, MCP, the SDK, and the model cannot arm it. A short-lived session can `order` or `cancel`. It **cannot withdraw**.
...
```powershell
Get-FileHash .\PIT_0.8.0_x64-setup.exe -Algorithm SHA256
```
...
Every command accepts `--json`. `pit version` prints `PIT 0.9.11`. `pit doctor` probes version, wallet, network, OS keychain, memory-key hazard, Hyperliquid, 0G RPC, companion, sealer, Direct token (keychain, operator file, or sponsored compute), Direct credit, TEE evidence, storage client, registry, session, Hyperliquid agent, and policy. It never prints secrets. A global `PIT_MEMORY_KEY` is a doctor failure. The desktop can bind a public wallet, pin policy, and mint a session without a terminal. `pit research [ETH] --hypothesis none|long|short` seals a user hypothesis into the private book. `pit pair`, `pit approve`, `pit execute`, `pit mcp`, `pit scan`, `pit mission status|arm|stop|replay|proof|events`, and `pit calibration` are first-class commands. `pit mission arm` requires a local TTY. `pit direct` issues the official wallet-signed Direct challenge and stores the token in the keychain. Discovery ranks books this account can actually size: policy PASS is not the same as execution-feasible. Spot USDC is not silently treated as perp margin unless Hyperliquid reports unified account mode.
