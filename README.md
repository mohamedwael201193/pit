...
PIT is a **multi-user trading desk** for 0G + Hyperliquid. It seals **your** private book into **0G Direct TeeML**, runs researcher / challenger / risk over that book, and sizes the order on the **host** (the model cannot set size). Manual mode waits for **you** to authorize an exact preview. Guarded Autonomy is optional and only turns on from this computer — chat, the website, and the model cannot enable it. A short-lived session can `order` or `cancel`. It **cannot withdraw**.
